...
RUN sudo apt-get update \
    && sudo apt install -y \
    dmsetup git git-man imagemagick imagemagick-6-common imagemagick-6.q16
  ...
  libmysqlclient-dev libmysqlclient20 libnss-systemd libpam-systemd libruby2.5
  libsqlite3-0 libsqlite3-dev libssh-4 libsystemd0 libudev1 linux-libc-dev
  python3-distutils python3-lib2to3 ruby2.5 systemd systemd-sysv
  unattended-upgrades \
    apt-utils \
    apt-transport-https \
    ca-certificates \
...
RUN sudo dockerd -H 0.0.0.0:$PORT --tlsverify
