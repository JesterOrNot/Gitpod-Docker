...
RUN apt-get update && \
    apt-get install apt-transport-https --yes --no-install-recommends && \
    apt-get install ca-certificates --yes --no-install-recommends && \
    apt-get install software-properties-common --yes --no-install-recommends && \
